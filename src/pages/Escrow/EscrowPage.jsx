...
const HOW_IT_WORKS = [
  ...
  { icon:'✅', title:'Deal complete',          desc:'DealMatch deducts its 3% facilitation fee and sends balance to landlord within 24 hours.' },
]
...
function EscrowRequestModal({ onClose }) {
  ...
  const platform_fee     = form.amount ? Math.round(Number(form.amount) * 0.03) : 0
  const landlordReceives = form.amount ? Number(form.amount) - platform_fee : 0
  ...
  const handleSubmit = async e => {
    ...
    await initializePaystackPayment({
      ...
      metadata: {
        ...
        duration: form.duration, platform_fee, landlord_receives: landlordReceives,
      },
      onSuccess: response => {
            ...
            {form.amount && (
              <div className="rounded-2xl p-4" style={{ backgroundColor:'rgba(122,158,126,0.08)', border:'1px solid rgba(122,158,126,0.2)' }}>
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between"><span style={{ color:'#8A7E78' }}>Platform Fee (3%)</span><strong style={{ color:'#C96A3A' }}>-₦{platform_fee.toLocaleString()}</strong></div>
